/*
 * End to End Test: One off payments via bank app (pay_by_bank)
 * Integrations tested
 * - Stripe
 * - Donorfy
 * - Mailchimp
...
const donorfyUK = getDonorfyClient("sandbox");
...
const testDetails = {
	...
	giftAid: true,
	preferences: {
		//email:true to test mailchimp (commented out for rate limiting)
		email: true,
		post: false,
		sms: true,
		phone: false,
...
test.describe("E2E: Test one off giving via Stripe", () => {
	test("Should test a successful pay by bank app payment", async ({ page }) => {
		const timestamp = Date.now();
		const testEmail = `james.holt+oneoffukbankapp${timestamp}@hopeforjustice.org`;
